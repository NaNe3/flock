...
export default function GroupLeave ({ navigation, route }) {
  ...
  const [color, setColor] = useState(gen.primaryColor)
                  ...
                  if (member.id !== userId) {
                    return (
                      ...
                      >
                        ...
                        <Icon name={newLeader === member.id ? 'circle-check' : 'circle'} size={20} color={newLeader === member.id ? color : gen.primaryText} />
                      </TouchableOpacity>
                    )
                  }
                })
